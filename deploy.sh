...
DEST_DIR="/Users/hesdx/Documents/dump/markdown-notebook-x"
PROJECT_DUMP=https://github.com/forzazero/markdown-notebook-x.git
...
# Check if it's a git repository/Users/hesdx/Documents/dump/tools-pgsetup
if [ ! -d ".git" ]; then
    echo "Warning: $DEST_DIR is not a git repository. Initializing..."
    git init
    git remote add origin $PROJECT_DUMP 2>/dev/null || true
fi
